...
The point is a short, memorable URL on the author's own domain that redirects to the installation script, so that the bootstrap ([#3](DOT-3.md)) does not start with cloning a repository by hand. The script behind the URL has to work when piped into `bash`, i.e. without a checkout, a TTY or the dotfiles already on `PATH`.
