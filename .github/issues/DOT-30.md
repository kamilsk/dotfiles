...
Because `cmm` joins every argument into the message ([#14](DOT-14.md)), the `--allow-empty` flag became part of the commit message instead of an option, and the commit failed on an empty tree (or, with staged files, was created with a nonsense message).
